...
def maxProductDifference(nums):
    ...
    # Create array without previous numbers
    int_arr = nums.copy()
    int_arr.remove(first_min)
    int_arr.remove(first_max)

    # Check the second largest and the second smallest number
    second_min = int_arr[0]
    second_max = 0
    for y in int_arr:
        if y < second_min:
            second_min = y
